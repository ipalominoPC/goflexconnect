...
interface FloorPlanUploadForFloorProps {
  floorId: string;
  onBack: () => void;
}

export default function FloorPlanUploadForFloor({ floorId, onBack }: FloorPlanUploadForFloorProps) {
  const floors = useStore((state) => state.floors);
  const updateFloor = useStore((state) => state.updateFloor);
  const floor = floors.find((f) => f.id === floorId);
  const existingFile = floor?.floorPlanImage || null;
  const [preview, setPreview] = useState<string | null>(existingFile);
  const [filename, setFilename] = useState<string | null>(floor?.floorPlanFilename || null);
