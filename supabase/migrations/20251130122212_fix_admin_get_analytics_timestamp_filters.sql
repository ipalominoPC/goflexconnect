/*
  # Fix admin_get_analytics timestamp comparisons
  
  1. Issue
    - measurements.timestamp and speed_tests.timestamp are timestamptz, not bigint
    - Comparing timestamptz >= bigint causes "operator does not exist" error
    
  2. Changes
    - Replace bigint millisecond comparisons with timestamptz comparisons
    - Use (now() - interval 'X') directly instead of converting to epoch milliseconds
    
  3. Fixed WHERE clauses
    - measurements: timestamp >= last_24h and timestamp >= last_7d
    - speed_tests: timestamp >= last_24h and timestamp >= last_7d
    - All other tables already use timestamptz correctly
*/
...
DECLARE
  ...
  -- Time boundaries (timestamptz only, no bigint)
...
BEGIN
  ...
  -- Check if caller is an approved admin (all emails from config)
  ...
  -- Measurements (FIXED: timestamp is timestamptz, not bigint)
  ...
  FROM measurements
  WHERE timestamp >= last_7d;
  
  -- Speed tests (FIXED: timestamp is timestamptz, not bigint)
  ...
  -- TOP HEAVY USERS (FIXED: timestamp comparisons)
